...
def compute_saliency_maps(X, y, model):
    ...
    x_gradients = X.grad.data
    abs_x_gradients = x_gradients.abs()
    saliency, _ = torch.max(abs_x_gradients, dim=1)
...
def make_fooling_image(X, target_y, model):
    ...
    for i in range(10):
      ...
      x_grad = X_fooling.grad.data

      dx = learning_rate * (x_grad / x_grad.norm())
      X_fooling.data += dx
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    dx = x_grad + 2 * l2_reg * img.data
    img.data += dx * learning_rate
