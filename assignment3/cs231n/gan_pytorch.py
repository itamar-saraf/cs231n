...
def sample_noise(batch_size, dim, seed=None):
    ...
    noise = torch.Tensor(batch_size, dim).uniform_(-1,1)
    return noise
...
def discriminator_loss(logits_real, logits_fake):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N = logits_real.shape
    real_label = torch.ones(N).type(dtype)
    fake_label = torch.zeros(N).type(dtype)
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    real_loss = torch.mean(torch.pow((scores_real - 1) ,2))
    fake_loss = torch.mean(torch.pow((scores_fake) ,2))
